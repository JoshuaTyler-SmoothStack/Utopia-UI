...
const CreateView = () => {
  ...
  const handleBookingUserGuestValidation = (userId, email, phone) => {
    ...
    const regexPhoneValidation = new RegExp(
      "^[+]*[(]{0,1}[0-9]{1,4}[)]{0,1}[-s./0-9]*$"
    );
    const validGuestEmail = regexEmailValidation.test(email);
    const validGuestPhone = regexPhoneValidation.test(phone);
    const isBookingUserGuestValid =
      (userId && !isNaN(userId)) || (validGuestEmail && validGuestPhone);
  ...
  const handleValidate = () => {
    ...
    if (
      !handleBookingUserGuestValidation(
        bookingUserId,
        bookingGuestEmail,
        bookingGuestPhone
      )
      ...
      {(status === "PENDING" || status === "ERROR") && (
        <FlexColumn className="mt-5">
          <ChangeOperationReadout
            ...
            name="Booking"
            result={String(`Booking ID: ${results.bookingId}\nConfirmation: ${results.bookingConfirmationCode}.`)}
          ...
          <ChangeOperationReadout
            ...
            result={
              results.bookingFlightId
                ? String(`Flight ID: ${results.bookingFlightId}.`)
                : "Unable to create Flight Booking."
            }
            status={results.bookingFlightId ? resultsStatus : "ERROR"}
          />
          ...
          <ChangeOperationReadout
            ...
            result={
              results.bookingGuestEmail
                ? String(`Email: ${results.bookingGuestEmail} Phone: ${results.bookingGuestPhone}.`)
                : bookingGuestEmail
          ...
          <ChangeOperationReadout
            ...
            result={
              results.bookingUserId
                ? String(`User ID: ${results.bookingUserId}.`)
                : bookingUserId
        ...
        <FlexColumn className="form-group row w-100">
          ...
          <FlexRow className="col-12 col-md-8 mt-3">
            <div style={{ width: "47.5%" }}>
              ...
              <input
                className={String(`form-control ${isSubmitted ? !bookingFlightId ? "is-invalid" : "is-valid" : ""}`)}
            ...
            <FlexColumn className="ml-auto" style={{ width: "47.5%" }}>
              <div>
                ...
                <input
                  ...
                  placeholder={
                    !bookingGuestEmail ? "" : "No Guest Email available."
                  }
                  type="email"
                  onChange={(e) => {
              ...
              <div className="mt-3">
                ...
                <input
                  ...
                  placeholder={
                    !bookingGuestPhone ? "" : "No Guest Phone available."
                  }
                  type="phone"
                  onChange={(e) => {
